...
const HomePage = () => {
  ...
  let getData = async (query = '') => {
    let response = await axios.get(`https://devioapi.pythonanywhere.com/users?query=${query}`)
  ...
  return (
    <div className='main--container'>
      <h2>Popular {total} Software Programmer found by @ratulhasan's webscraper</h2>
      ...
      <div className='user__list'>
        { users.map((user, index) =>(
          <div className='user__preview__wrapper' key={index}>
              ...
              <small className='text-center'>at { user.company['name']}</small>
